...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
   ...
   """
    ...
    fact = 0.9
    mom_params = []
    for mom_param,gparam in zip(classifier.params_momentum,gparams):
        last_update = gparam - fact * mom_param
        mom_params.append(last_update)
    ################################################################
    ...
    for param, mom_param in zip(classifier.params, mom_params):
        updates.append((param, param - learning_rate * mom_param))
    #################################################################
    for mom_param,param in zip(classifier.params_momentum,mom_params):
        updates.append((mom_param,param))
    #################################################################
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            print "epoch = %i mini batch = %i Average cost is %f \n"%(epoch,minibatch_index, minibatch_avg_cost)
            # iteration number
